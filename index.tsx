...
interface Recipe {
  ...
  servings: string;
  cookingTime: string;
  ingredients: { subHeading?: string; items: string[] }[];
  instructions: string[];
  description?: string;
}
...
interface ShoppingListItem {
  recipeInfo: Pick<Recipe, 'id' | 'recipeName'>;
  items: Set<string>;
}
...
const input = document.querySelector<HTMLTextAreaElement>('#ingredients');
...
const suggestionButtons = document.querySelector<HTMLDivElement>('#suggestions');
const voiceInputBtn = document.querySelector<HTMLButtonElement>('#voice-input-btn');
const voiceError = document.querySelector<HTMLDivElement>('#voice-error');
...
const skeletonLoader = document.querySelector<HTMLDivElement>('#skeleton-loader');
const recipeListView = document.querySelector<HTMLDivElement>('#recipe-list-view');
const recipeDetailView = document.querySelector<HTMLDivElement>('#recipe-detail-view');
...
const favoritesContainer = document.querySelector<HTMLDivElement>('#favorites-container');
const shoppingListContainer = document.querySelector<HTMLDivElement>('#shopping-list-container');
const shoppingListActions = document.querySelector<HTMLDivElement>('#shopping-list-actions');
...
let shoppingList: Map<string, ShoppingListItem> = new Map();
...
const loadStateFromLocalStorage = () => {
    try {
        ...
        if (storedShoppingList) {
            ...
            parsed.forEach((item: [string, ShoppingListItem]) => {
...
const showRecipeDetailView = (recipe: Recipe, origin: 'search' | 'favorites') => {
    if(!recipeListView || !recipeDetailView) return;
    ...
    document.getElementById('add-to-shopping-list-from-detail')?.addEventListener('click', () => {
        ...
        const button = document.getElementById('add-to-shopping-list-from-detail') as HTMLButtonElement;
...
const callGeminiAPI = async (query: SearchQuery): Promise<Recipe[] | null> => {
    ...
    try {
        ...
        return recipes.map((recipe: Omit<Recipe, 'id'>) => ({ ...recipe, id: `recipe-${Date.now()}-${Math.random()}` }));
...
const handleFormSubmit = async (event: Event) => {
    ...
    if (!form || !input || !servingsInput || !mealPrepToggle || !allergiesInput || !skeletonLoader || !resultsContainer) return;
